...
class RtcClient {
  ...
  async join() {
    ...
    try {
      // join the room
      await this.client_.join({ roomId: this.roomId_, privateMapKey: this.privateMapKey_ });
      console.log('join room success');
      // ('进房成功！');
      this.isJoined_ = true;
      window.checkremote && clearTimeout(window.checkremote);
      // 10s后检测用户是否接通，否则直接中断
      window.checkremote = setTimeout(function() {
        if (window.calling && !window.onlineNum) {
          openMyAlert('客户端未接通或网络异常，通话中断', function() {
            if (window.calling) {
    ...
    try {
      ...
      try {
        ...
        console.error('createLocalStream with audio failed: ' + error);
        // return;
      }
    }
    ...
    this.localStream_.play(this.playDiv_ || 'local_stream');
  ...
  async createLocalStream(options) {
    ...
    this.localStream_.setAudioProfile('high');

    await this.localStream_.initialize();

    // TRTC.getMicrophones().then((devices) => {
    //   this.localStream_.switchDevice('audio', devices[0]).then(() => {});
    // });
  }
  ...
  handleEvents() {
    ...
    this.client_.on('error', err => {
      ...
      if (!window.navigator.onLine) {
        ...
        this.leave();
        openMyAlert('连接错误：' + err);
      }
      // if (confirm('连接失败：'+ err +'，是否尝试重新连接？')) {
      //   this.join();
      // } else {
      $$('.backdrop-in').remove();
      // 连接失败，直接中断通话
      if (window.calling && window.navigator.onLine) {
        handupCallByReason(false, function() {
          window.calling = false;
          closeVedioRequest();
        });
    ...
    this.client_.on('peer-join', evt => {
      ...
      window.onlineNum ++;
      // if (window.onlineNum >=2 ) {
      //   alert('视频通话正在连通中...，不可超过两人加入');
      //   $$('.leaveBtn').click();
      // }
      // ('远端用户进房 - ' + userId);
    });
    ...
    this.client_.on('stream-added', evt => {
      ...
      $$('.hangup').append('<span class="call-time">00:00</span>');
      // 通知已经接通
      
      window.curtime = window.curtime || new Date().getTime(); 
      window.callInterval && clearInterval(window.callInterval);
      window.callInterval = setInterval(function() {
        if ($$('.call-time')) {
          var difftimes = new Date().getTime() - window.curtime;
          var duringStr = formatCallDuring(difftimes);
          $$('.call-time').html(duringStr);
        }
      }, 1000);
    });
    ...
    this.client_.on('stream-removed', evt => {
      ...
      // ('远端流删除 - ' + remoteStream.getUserId());
      console.log('远端流删除！' + remoteStream.getUserId());
      if (remoteStream.getUserId() != this.userId_) {
        console.log('BBBB ' + this.userId_);
      }
    });
    ...
    // 远端流音频或视频mute状态通知
    this.client_.on('mute-audio', evt => { // 远端用户禁用音频通知
      console.log(evt.userId + ' mute audio');
    });
    this.client_.on('unmute-audio', evt => { // 远端用户禁用视频通知
